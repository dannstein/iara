...
public record SolicitacaoServicoDTO(
    ...
    public static SolicitacaoServicoDTO from(SolicitacaoServico s, List<String> pdfUrls) {
        return new SolicitacaoServicoDTO(
                s.getId(), s.getUsuario().getId(), s.getTipo(), s.getEnderecoTxt(),
                GeoUtil.toCoordenadas(s.getGeometria()), s.getDescricaoMotivo(), s.getFotosUrls(),
                s.getStatus(), s.getObservacaoDc(), pdfUrls, s.getCreatedAt()
        );
    }
}
